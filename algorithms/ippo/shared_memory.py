...
def initSharedMemory(algorithm):
    ...
    Parameters:
        ...
        value_shm (shared_memory): Shared Memory Block for Value Function
        advantages_shm (shared_memory): Shared Memory Block for Advantages
        log_prob_shm (shared_memory): Shared Memory Block for Log Probabilities
    ...
    ac_shm = shm.SharedMemory(create=True, size=Config.buffer_length * sum(1 if isinstance(env.action_space, Box) else 1 for i in range(Config.n_agents)) * np.dtype(np.float64).itemsize)
    ...
    value_shm = shm.SharedMemory(create=True, size=Config.buffer_length * Config.n_agents * np.dtype(np.float64).itemsize)
    advantages_shm = shm.SharedMemory(create=True, size=Config.buffer_length * Config.n_agents * np.dtype(np.float64).itemsize)
    log_prob_shm = shm.SharedMemory(create=True, size=Config.buffer_length * Config.n_agents * np.dtype(np.float64).itemsize)
    ...
    return {
        ...
        'done_shm': done_shm,
        'value_shm': value_shm,
        'advantages_shm': advantages_shm,
        'log_prob_shm': log_prob_shm,
        'curr_i': curr_i,
        'filled_i': filled_i,
        'fin_dc': fin_dc,
        'fin_mu': fin_mu,
...
class ReplayBuffer(object):
    ...
    """
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims,obs_shm, ac_shm, rew_shm, next_obs_shm, done_shm, value_shm, advantages_shm, log_prob_shm, curr_i, filled_i):
        """
        Inputs:
        ...
        self.value_shm = value_shm
        self.advantages_shm = advantages_shm
        self.log_prob_shm = log_prob_shm
        ...
        self.val_buffs = np.ndarray((max_steps, num_agents), dtype=np.float32, buffer=self.value_shm.buf)
        self.adv_buffs = np.ndarray((max_steps, num_agents), dtype=np.float32, buffer=self.advantages_shm.buf)
        self.log_prob_buffs = np.ndarray((max_steps, num_agents), dtype=np.float32, buffer=self.log_prob_shm.buf)
    ...
    def push(self, obs, actions, rewards, next_obs, dones, values, log_probs):
        ...
        if self.curr_i[0] + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.val_buffs[:, agent_i] = np.roll(self.val_buffs[:, agent_i], rollover)
                self.log_prob_buffs[:, agent_i] = np.roll(self.log_prob_buffs[:, agent_i], rollover)
            self.curr_i[0] = 0
            self.filled_i[0] = self.max_steps
        for agent_i in range(self.num_agents):
            self.obs_buffs[agent_i][self.curr_i[0]:self.curr_i[0] + nentries] = np.vstack(observations[:, agent_i])
            self.ac_buffs[agent_i][self.curr_i[0]:self.curr_i[0] + nentries] = actions[:, agent_i].reshape(-1, 1)
            ...
            self.val_buffs[self.curr_i[0]:self.curr_i[0] + nentries, agent_i] = values[:, agent_i]
            self.log_prob_buffs[self.curr_i[0]:self.curr_i[0] + nentries, agent_i] = log_probs[:, agent_i]
    ...
    def sample(self, a_i, N, device='cpu', norm_rews=False, rank = 0):
        ...
        return (cast(self.obs_buffs[a_i][inds]),
                ...
                cast(self.val_buffs[inds,a_i]),
                cast(self.log_prob_buffs[inds,a_i]),
                cast(self.adv_buffs[inds,a_i]))
