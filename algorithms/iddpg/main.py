...
def main_actors(iddpg, shared_memory_params):
    ...
    replay_buffer = ReplayBuffer(Config.buffer_length, Config.n_agents,
                                ...
                                [env.action_space.shape[0] if isinstance(env.action_space, Box) else env.action_space.n for i in range(Config.n_agents)],
    ...
    mu_time = 0
    mu_count = 0
    iddpg.prep_rollouts(device='cpu')
    first_mu_time = True
    first_dc_time = True
    # Load each agent's model parameters from shared memory
    for ep_i in range(0, Config.n_episodes, Config.n_rollout_threads):
        ...
        for et_i in range(Config.episode_length):
            ...
            one_hot_action = np.array([action.numpy() for action in torch_agent_actions])
            actions = [
            ...
            replay_buffer.push(obs, one_hot_action, rewards, next_obs, terminated)
            obs = next_obs
...
def main_learners(rank, iddpg, world_size, shared_memory_params, lock):
    ...
    while(1):
        ...
        # Model Update
        ### 
        iddpg.prep_training(start_idx, end_idx, device=get_device(rank))
        dist.barrier()
        ...
        for a_i in range(start_idx, end_idx):
            for u_i in range(Config.n_updates):
                ...
                iddpg.update(sample, a_i, device=get_device(rank))
            iddpg.update_all_targets(a_i)
        ...
        if start_idx != previous_start_idx or end_idx != previous_end_idx:
            load_agents_from_shared_memory(iddpg.agents, shared_memory_params['shared_memory_blocks'], shared_memory_params['shared_mem'], start_idx, end_idx)
...
if __name__ == '__main__':
    ...
    shared_memory_params = initSharedMemory(iddpg)

    # spawn actors 
    ap = ctx.Process(target = main_actors,
             args=(iddpg, shared_memory_params))

    learner_processes = [ctx.Process(target = main_learners,
             args=(i, iddpg, world_size_learners, shared_memory_params, lock))
